...
- `mvn clean install` — compile, test, assemble the jar (`target/moleculer-java-jmx-2.1.1.jar`) and install to the local `~/.m2` repository
...
- Version lives in two places in `pom.xml`: the project `<version>` and the `moleculer-java`
  dependency `<version>` (currently the `${moleculer.version}` property). Both are `2.1.1`
